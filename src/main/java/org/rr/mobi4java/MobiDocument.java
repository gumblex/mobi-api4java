...
import org.rr.mobi4java.decompression.HuffCdic;
import org.rr.mobi4java.decompression.Inflater;
import org.rr.mobi4java.decompression.None;
import org.rr.mobi4java.decompression.PlamDoc;
...
public class MobiDocument {
    ...
    /**
     * Store the given image data as cover. If the document already have a cover,
     * the old cover will be replaced with the given one.
     * The thumbnail for this document is not affected by this method.
     * The thumbnail must be set separately with the {@link #setThumbnail(byte[])} method.
     *
     * @param image The image bytes to be used as cover.
     *              Take sure that the image format is supported by the target device.
    ...
    /**
     * Store the given image data as thumbnail. If the document already have a thumbnail,
     * the old thumbnail will be replaced with the given one.
     *
     * @param image The image bytes to be used as thumbnail.
     *              Take sure that the image format is supported by the target device.
     * @throws IllegalArgumentException if the given image is null.
     */
    ...
    List<MobiContent> getImageContents() {
        ...
        for (int i = firstImageIndex; i < mobiContents.size(); i++) {
            ...
            if (content.getType() == CONTENT_TYPE.IMAGE
                    || content.getType() == CONTENT_TYPE.COVER
                    || content.getType() == CONTENT_TYPE.THUMBNAIL) {
    ...
    /**
     * Get the character encoding for the {@link MobiDocument}.
     * The returned value must be a valid value like 'UTF-8', 'UTF-16' or 'Cp1252'
     *
    ...
    /**
     * Removes the old text content and applies the given text to the {@link MobiDocument}.
     * If a book index exists it will be removed.
     *
    ...
    private void adjustIndices(int firstNonBookIndex, int firstImageIndex) {
        ...
        mobiHeader.setFcisRecordIndex(
                MobiUtils.findFirstContentsIndexByType(mobiContents, MobiContent.CONTENT_TYPE.FCIS));
        mobiHeader.setFlisRecordIndex(
                MobiUtils.findFirstContentsIndexByType(mobiContents, MobiContent.CONTENT_TYPE.FLIS));
        mobiHeader.setSrcsRecordIndex(
                MobiUtils.findFirstContentsIndexByType(mobiContents, MobiContent.CONTENT_TYPE.SRCS));
        mobiHeader.setDatpRecordIndex(
                MobiUtils.findFirstContentsIndexByType(mobiContents, MobiContent.CONTENT_TYPE.DATP));
    ...
    private void adjustExthOffsets(RECORD_TYPE recordType, CONTENT_TYPE contentType, int firstImageIndex) {
        ...
        if (exthRecords.size() != indices.size()) {
            throw new IllegalArgumentException(
                    String.format("Found %s thumbnail header but %s really exists.",
                            exthRecords.size(), indices.size()));
        }
    ...
    private Inflater getUnPacker() throws IllegalArgumentException {
        switch (mobiHeader.getCompressionCode()) {
            case NONE:
                return new None();
            case PALM_DOC:
                return new PlamDoc();
            case HUFF_CDIC:
                return new HuffCdic( mobiContents);
            default:
