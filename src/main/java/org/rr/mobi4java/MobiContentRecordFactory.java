...
class MobiContentRecordFactory {
    ...
    static CONTENT_TYPE evaluateType(PDBHeader pdbHeader, MobiContentHeader mobiHeader, int index, byte[] mobiData,
                                     long recordDataOffset, long recordDataLength) {
        ...
        } else if (isHuffman(content)) {
            return CONTENT_TYPE.HUFF;
        } else if (isCdic(content)) {
            return CONTENT_TYPE.CDIC;
        }
    ...
    /**
     * Tries to guess what the image type (if any) of a file based on the file's "magic numbers,"
     ...
     * Each media file is stored in a separate AUDI (audio) or VIDE
     * (video) record
     */
    ...
    private static boolean isHuffman(byte[] content) {
        return startsWith(content, "HUFF".getBytes());
    }

    private static boolean isCdic(byte[] content) {
        return startsWith(content, "CDIC".getBytes());
    }
}
